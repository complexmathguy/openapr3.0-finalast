...
public class ReportPayloadDescriptorEntityProjector {
    ...
    public List<ReportPayloadDescriptor> findAll( FindAllReportPayloadDescriptorQuery query ) {
    	...
    	try {
    		...
    		LOGGER.log( Level.WARNING, "Failed to find all ReportPayloadDescriptor - {0}", exc.getMessage() );
    	}
    	return null;
    }
    ...
    // --------------------------------------------------
	@Autowired
    protected final ReportPayloadDescriptorRepository repository;

    private static final Logger LOGGER 	= Logger.getLogger(ReportPayloadDescriptorEntityProjector.class.getName());
