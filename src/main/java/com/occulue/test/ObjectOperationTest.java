...
public class ObjectOperationTest{
	...
	protected void jumpStart() throws Throwable {
		...
		objectOperationId = ObjectOperationBusinessDelegate.getObjectOperationInstance()
	...
	protected void setUpQuerySubscriptions() throws Throwable {
		...
		try {            
			subscriber.objectOperationSubscribe().updates().subscribe(
					  successValue -> {
						  LOGGER.info(successValue.toString());
						  try {
							  LOGGER.info("GetAll update received for ObjectOperation : " + successValue.getObjectOperationId());
							  if (successValue.getObjectOperationId().equals(objectOperationId)) {
								  if (testingStep.equals("create")) {
									  ...
									  state( getAll().size() == sizeOfObjectOperationList - 1 , "value not deleted from list");
									  LOGGER.info("**********************************************************");
			...
			subscriber.objectOperationSubscribe( objectOperationId ).updates().subscribe(
					  successValue -> {
						  LOGGER.info(successValue.toString());
						  try {
							  LOGGER.info("GetOne update received for ObjectOperation : " + successValue.getObjectOperationId() + " in step " + testingStep);
							  testingStep = "delete";
	...
	protected ObjectOperation read() throws Throwable {
		...
		ObjectOperationFetchOneSummary fetchOneSummary = new ObjectOperationFetchOneSummary( objectOperationId );

		try {
			entity = ObjectOperationBusinessDelegate.getObjectOperationInstance().getObjectOperation( fetchOneSummary );
	...
	protected void update() throws Throwable {
		...
		try {            
			...
			ObjectOperationBusinessDelegate proxy = ObjectOperationBusinessDelegate.getObjectOperationInstance();  
			...
			LOGGER.warning( unexpectedErrorMsg );
			LOGGER.warning( msg.toString() + " : primarykey = " + objectOperationId + " : command -" +  command + " : " + e );
	...
	protected void delete() throws Throwable {
		...
		try{
			ObjectOperationBusinessDelegate.getObjectOperationInstance().delete( new DeleteObjectOperationCommand( entity.getObjectOperationId() ) ).get();
	...
	protected List<ObjectOperation> getAll() throws Throwable {    
		...
		try {
			...
			collection = ObjectOperationBusinessDelegate.getObjectOperationInstance().getAllObjectOperation();
			...
			while( iter.hasNext() ) {
				...
				assertNotNull( entity.getObjectOperationId(), "-- entity in Collection has a null primary key" );        
	...
	protected CreateObjectOperationCommand generateNewCommand() {
        CreateObjectOperationCommand command = new CreateObjectOperationCommand( null,  org.apache.commons.lang3.RandomStringUtils.randomAlphanumeric(16),  org.apache.commons.lang3.RandomStringUtils.randomAlphanumeric(16),  ObjectType.values()[0],  Operation.values()[0] );
	...
	protected UpdateObjectOperationCommand generateUpdateCommand() {
	        UpdateObjectOperationCommand command = new UpdateObjectOperationCommand( null,  org.apache.commons.lang3.RandomStringUtils.randomAlphanumeric(16),  org.apache.commons.lang3.RandomStringUtils.randomAlphanumeric(16),  ObjectType.values()[0],  Operation.values()[0] );
