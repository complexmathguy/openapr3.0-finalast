...
public class NotifierTest{
	...
	protected void jumpStart() throws Throwable {
		...
		notifierId = NotifierBusinessDelegate.getNotifierInstance()
		.createNotifier( generateNewCommand() )
		.get();
	...
	protected void setUpQuerySubscriptions() throws Throwable {
		...
		try {            
			subscriber.notifierSubscribe().updates().subscribe(
					  successValue -> {
						  LOGGER.info(successValue.toString());
						  try {
							  LOGGER.info("GetAll update received for Notifier : " + successValue.getNotifierId());
							  if (successValue.getNotifierId().equals(notifierId)) {
								  if (testingStep.equals("create")) {
									  ...
									  state( getAll().size() == sizeOfNotifierList - 1 , "value not deleted from list");
									  LOGGER.info("**********************************************************");
			...
			subscriber.notifierSubscribe( notifierId ).updates().subscribe(
					  successValue -> {
						  LOGGER.info(successValue.toString());
						  try {
							  LOGGER.info("GetOne update received for Notifier : " + successValue.getNotifierId() + " in step " + testingStep);
							  testingStep = "delete";
	...
	protected void update() throws Throwable {
		...
		try {            
			...
			LOGGER.warning( unexpectedErrorMsg );
			LOGGER.warning( msg.toString() + " : primarykey = " + notifierId + " : command -" +  command + " : " + e );
	...
	protected void delete() throws Throwable {
		...
		try{
			NotifierBusinessDelegate.getNotifierInstance().delete( new DeleteNotifierCommand( entity.getNotifierId() ) ).get();
