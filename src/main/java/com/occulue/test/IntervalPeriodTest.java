...
public class IntervalPeriodTest{
	...
	protected void jumpStart() throws Throwable {
		...
		intervalPeriodId = IntervalPeriodBusinessDelegate.getIntervalPeriodInstance()
	...
	protected void setUpQuerySubscriptions() throws Throwable {
		...
		try {            
			subscriber.intervalPeriodSubscribe().updates().subscribe(
					  successValue -> {
						  LOGGER.info(successValue.toString());
						  try {
							  LOGGER.info("GetAll update received for IntervalPeriod : " + successValue.getIntervalPeriodId());
							  if (successValue.getIntervalPeriodId().equals(intervalPeriodId)) {
								  if (testingStep.equals("create")) {
									  ...
									  state( getAll().size() == sizeOfIntervalPeriodList - 1 , "value not deleted from list");
									  LOGGER.info("**********************************************************");
			...
			subscriber.intervalPeriodSubscribe( intervalPeriodId ).updates().subscribe(
					  successValue -> {
						  LOGGER.info(successValue.toString());
						  try {
							  LOGGER.info("GetOne update received for IntervalPeriod : " + successValue.getIntervalPeriodId() + " in step " + testingStep);
							  testingStep = "delete";
	...
	protected IntervalPeriod read() throws Throwable {
		...
		IntervalPeriodFetchOneSummary fetchOneSummary = new IntervalPeriodFetchOneSummary( intervalPeriodId );

		try {
			entity = IntervalPeriodBusinessDelegate.getIntervalPeriodInstance().getIntervalPeriod( fetchOneSummary );
	...
	protected void update() throws Throwable {
		...
		try {            
			...
			IntervalPeriodBusinessDelegate proxy = IntervalPeriodBusinessDelegate.getIntervalPeriodInstance();  
			...
			LOGGER.warning( unexpectedErrorMsg );
			LOGGER.warning( msg.toString() + " : primarykey = " + intervalPeriodId + " : command -" +  command + " : " + e );
	...
	protected void delete() throws Throwable {
		...
		try{
			IntervalPeriodBusinessDelegate.getIntervalPeriodInstance().delete( new DeleteIntervalPeriodCommand( entity.getIntervalPeriodId() ) ).get();
	...
	protected List<IntervalPeriod> getAll() throws Throwable {    
		...
		try {
			...
			collection = IntervalPeriodBusinessDelegate.getIntervalPeriodInstance().getAllIntervalPeriod();
			...
			while( iter.hasNext() ) {
				...
				assertNotNull( entity.getIntervalPeriodId(), "-- entity in Collection has a null primary key" );        
