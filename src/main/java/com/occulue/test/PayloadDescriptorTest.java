...
public class PayloadDescriptorTest{
	...
	protected void jumpStart() throws Throwable {
		...
		payloadDescriptorId = PayloadDescriptorBusinessDelegate.getPayloadDescriptorInstance()
	...
	protected void setUpQuerySubscriptions() throws Throwable {
		...
		try {            
			subscriber.payloadDescriptorSubscribe().updates().subscribe(
					  successValue -> {
						  LOGGER.info(successValue.toString());
						  try {
							  LOGGER.info("GetAll update received for PayloadDescriptor : " + successValue.getPayloadDescriptorId());
							  if (successValue.getPayloadDescriptorId().equals(payloadDescriptorId)) {
								  if (testingStep.equals("create")) {
									  ...
									  state( getAll().size() == sizeOfPayloadDescriptorList - 1 , "value not deleted from list");
									  LOGGER.info("**********************************************************");
			...
			subscriber.payloadDescriptorSubscribe( payloadDescriptorId ).updates().subscribe(
					  successValue -> {
						  LOGGER.info(successValue.toString());
						  try {
							  LOGGER.info("GetOne update received for PayloadDescriptor : " + successValue.getPayloadDescriptorId() + " in step " + testingStep);
							  testingStep = "delete";
					  ...
					  error -> LOGGER.warning(error.getMessage()),
					  () -> LOGGER.info("Subscription on payloadDescriptor for payloadDescriptorId consumed")
	...
	protected PayloadDescriptor read() throws Throwable {
		...
		PayloadDescriptorFetchOneSummary fetchOneSummary = new PayloadDescriptorFetchOneSummary( payloadDescriptorId );

		try {
			entity = PayloadDescriptorBusinessDelegate.getPayloadDescriptorInstance().getPayloadDescriptor( fetchOneSummary );
	...
	protected void update() throws Throwable {
		...
		try {            
			...
			PayloadDescriptorBusinessDelegate proxy = PayloadDescriptorBusinessDelegate.getPayloadDescriptorInstance();  
			...
			LOGGER.warning( unexpectedErrorMsg );
			LOGGER.warning( msg.toString() + " : primarykey = " + payloadDescriptorId + " : command -" +  command + " : " + e );
	...
	protected void delete() throws Throwable {
		...
		try{
			PayloadDescriptorBusinessDelegate.getPayloadDescriptorInstance().delete( new DeletePayloadDescriptorCommand( entity.getPayloadDescriptorId() ) ).get();
	...
	protected List<PayloadDescriptor> getAll() throws Throwable {    
		...
		try {
			...
			collection = PayloadDescriptorBusinessDelegate.getPayloadDescriptorInstance().getAllPayloadDescriptor();
			assertNotNull( collection, "An Empty collection of PayloadDescriptor was incorrectly returned.");
			...
			while( iter.hasNext() ) {
				...
				assertNotNull( entity.getPayloadDescriptorId(), "-- entity in Collection has a null primary key" );        
	...
	protected CreatePayloadDescriptorCommand generateNewCommand() {
        CreatePayloadDescriptorCommand command = new CreatePayloadDescriptorCommand( null,  org.apache.commons.lang3.RandomStringUtils.randomAlphanumeric(16),  ObjectType.values()[0] );
	...
	protected UpdatePayloadDescriptorCommand generateUpdateCommand() {
	        UpdatePayloadDescriptorCommand command = new UpdatePayloadDescriptorCommand( null,  org.apache.commons.lang3.RandomStringUtils.randomAlphanumeric(16),  ObjectType.values()[0] );
