...
public class ReportDescriptorAggregate {  
    ...
    public ReportDescriptorAggregate(CreateReportDescriptorCommand command) throws Exception {
    	...
    	CreateReportDescriptorEvent event = new CreateReportDescriptorEvent(command.getReportDescriptorId(), command.getPayloadType(), command.getReadingType(), command.getUnits(), command.getAggregate(), command.getStartInterval(), command.getNumIntervals(), command.getHistorical(), command.getFrequency(), command.getRepeat());
    ...
    public void handle(AssignTargetsToReportDescriptorCommand command) throws Exception {
    	...
    	if (  targets != null && targets.getValuesMapId() == command.getAssignment().getValuesMapId() )
    		throw new ProcessingException( "Targets already assigned with id " + command.getAssignment().getValuesMapId() );  
    		
        apply(new AssignTargetsToReportDescriptorEvent(command.getReportDescriptorId(), command.getAssignment()));
    }
    ...
    public void handle(UnAssignTargetsFromReportDescriptorCommand command) throws Exception {
    	...
    	if (  targets == null )
    		throw new ProcessingException( "Targets already has nothing assigned." );  
