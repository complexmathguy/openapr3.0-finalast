 ...
 * <li>Improving the available of PayloadDescriptor related services in the case of a PayloadDescriptor business related service failing.</li>
 * <li>Exposes a simpler, uniform PayloadDescriptor interface to the business tier, making it easy for clients to consume a simple Java object.</li>
 * <li>Hides the communication protocol that may be required to fulfill PayloadDescriptor business related services.</li>
 * </ol>
 * <p>
 * @author your_name_here
 */
public class PayloadDescriptorBusinessDelegate 
    ...
    */
    ...
    throws ProcessingException, IllegalArgumentException {
		...
		try {
    		...
    		// ---------------------------------------
    		// issue the CreatePayloadDescriptorCommand - by convention the future return value for a create command
        	// that is handled by the constructor of an aggregate will return the UUID 
    		// ---------------------------------------
        	completableFuture = commandGateway.send( command );
        	
			LOGGER.log( Level.INFO, "return from Command Gateway for CreatePayloadDescriptorCommand of PayloadDescriptor is " + command );
    ...
    */
    ...
    throws ProcessingException, IllegalArgumentException {	
        ...
        try {  
            ...
            final String errMsg = "Unable to delete PayloadDescriptor using Id = "  + command.getPayloadDescriptorId();
            LOGGER.log( Level.WARNING, errMsg, exc );
            throw new ProcessingException( errMsg, exc );
        }
        finally {
    ...
    throws ProcessingException, IllegalArgumentException {
        ...
        try {
        	...
        	// --------------------------------------
        	CompletableFuture<PayloadDescriptor> futureEntity = queryGateway.query(new FindPayloadDescriptorQuery( new LoadPayloadDescriptorFilter( summary.getPayloadDescriptorId() ) ), ResponseTypes.instanceOf(PayloadDescriptor.class));
        	
        	entity = futureEntity.get();
        }
        catch( Exception exc ) {
            final String errMsg = "Unable to locate PayloadDescriptor with id " + summary.getPayloadDescriptorId();
            LOGGER.log( Level.WARNING, errMsg, exc );
            throw new ProcessingException( errMsg, exc );
        }
        finally {
     ...
     */
    public List<PayloadDescriptor> getAllPayloadDescriptor() 
    throws ProcessingException {
        List<PayloadDescriptor> list = null;

        try {
        	CompletableFuture<List<PayloadDescriptor>> futureList = queryGateway.query(new FindAllPayloadDescriptorQuery(), ResponseTypes.multipleInstancesOf(PayloadDescriptor.class));
	 ...
	 */
	protected PayloadDescriptor load( UUID id ) throws ProcessingException {
		payloadDescriptor = PayloadDescriptorBusinessDelegate.getPayloadDescriptorInstance().getPayloadDescriptor( new PayloadDescriptorFetchOneSummary(id) );	
		return payloadDescriptor;
	}
...
//************************************************************************
    ...
    private static final Logger LOGGER 			= Logger.getLogger(PayloadDescriptorBusinessDelegate.class.getName());
