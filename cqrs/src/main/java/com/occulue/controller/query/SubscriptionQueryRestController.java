...
public class SubscriptionQueryRestController extends BaseSpringRestController {
    ...
    public Subscription load( @RequestParam(required=true) UUID subscriptionId ) {
    	...
    	try {  
    		entity = SubscriptionBusinessDelegate.getSubscriptionInstance().getSubscription( new SubscriptionFetchOneSummary( subscriptionId ) );
        }
        catch( Throwable exc ) {
    ...
    public List<Subscription> loadAll() {                
    	...
    	try {
            ...
            subscriptionList = SubscriptionBusinessDelegate.getSubscriptionInstance().getAllSubscription();
