...
public class EventPayloadDescriptorQueryRestController extends BaseSpringRestController {
    ...
    public EventPayloadDescriptor load( @RequestParam(required=true) UUID eventPayloadDescriptorId ) {
    	...
    	try {  
    		entity = EventPayloadDescriptorBusinessDelegate.getEventPayloadDescriptorInstance().getEventPayloadDescriptor( new EventPayloadDescriptorFetchOneSummary( eventPayloadDescriptorId ) );
        }
        catch( Throwable exc ) {
            LOGGER.log( Level.WARNING, "failed to load EventPayloadDescriptor using Id " + eventPayloadDescriptorId );
            return null;
        }
    ...
    public List<EventPayloadDescriptor> loadAll() {                
    	...
    	try {
            ...
            eventPayloadDescriptorList = EventPayloadDescriptorBusinessDelegate.getEventPayloadDescriptorInstance().getAllEventPayloadDescriptor();
...
//************************************************************************
    ...
    private static final Logger LOGGER = Logger.getLogger(EventPayloadDescriptorQueryRestController.class.getName());
