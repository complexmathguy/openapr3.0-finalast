...
public class NotifierQueryRestController extends BaseSpringRestController {
    ...
    public Notifier load( @RequestParam(required=true) UUID notifierId ) {
    	...
    	try {  
    		entity = NotifierBusinessDelegate.getNotifierInstance().getNotifier( new NotifierFetchOneSummary( notifierId ) );
        }
        catch( Throwable exc ) {
    ...
    public List<Notifier> loadAll() {                
    	...
    	try {
            ...
            if ( notifierList != null )
                LOGGER.log( Level.INFO,  "successfully loaded all Notifiers" );
        }
        catch( Throwable exc ) {
...
//************************************************************************
    ...
    private static final Logger LOGGER = Logger.getLogger(NotifierQueryRestController.class.getName());
