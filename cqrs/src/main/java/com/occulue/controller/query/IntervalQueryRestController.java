...
public class IntervalQueryRestController extends BaseSpringRestController {
    ...
    public Interval load( @RequestParam(required=true) UUID intervalId ) {
    	...
    	try {  
    		entity = IntervalBusinessDelegate.getIntervalInstance().getInterval( new IntervalFetchOneSummary( intervalId ) );
        }
        catch( Throwable exc ) {
    ...
    public List<Interval> loadAll() {                
    	...
    	try {
            ...
            if ( intervalList != null )
                LOGGER.log( Level.INFO,  "successfully loaded all Intervals" );
        }
        catch( Throwable exc ) {
...
//************************************************************************
    ...
    private static final Logger LOGGER = Logger.getLogger(IntervalQueryRestController.class.getName());
