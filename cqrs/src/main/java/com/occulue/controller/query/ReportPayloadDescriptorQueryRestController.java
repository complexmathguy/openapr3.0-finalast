...
public class ReportPayloadDescriptorQueryRestController extends BaseSpringRestController {
     ...
     */    
    @GetMapping("/load")
    public ReportPayloadDescriptor load( @RequestParam(required=true) UUID reportPayloadDescriptorId ) {
    	ReportPayloadDescriptor entity = null;

    	try {  
    		entity = ReportPayloadDescriptorBusinessDelegate.getReportPayloadDescriptorInstance().getReportPayloadDescriptor( new ReportPayloadDescriptorFetchOneSummary( reportPayloadDescriptorId ) );
        }
        catch( Throwable exc ) {
            LOGGER.log( Level.WARNING, "failed to load ReportPayloadDescriptor using Id " + reportPayloadDescriptorId );
            return null;
        }
    ...
    public List<ReportPayloadDescriptor> loadAll() {                
    	...
    	try {
            ...
            reportPayloadDescriptorList = ReportPayloadDescriptorBusinessDelegate.getReportPayloadDescriptorInstance().getAllReportPayloadDescriptor();
...
//************************************************************************
    ...
    private static final Logger LOGGER = Logger.getLogger(ReportPayloadDescriptorQueryRestController.class.getName());
