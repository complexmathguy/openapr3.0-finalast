...
public class ProblemQueryRestController extends BaseSpringRestController {
    ...
    public Problem load( @RequestParam(required=true) UUID problemId ) {
    	...
    	try {  
    		entity = ProblemBusinessDelegate.getProblemInstance().getProblem( new ProblemFetchOneSummary( problemId ) );
        }
        catch( Throwable exc ) {
    ...
    public List<Problem> loadAll() {                
    	...
    	try {
            ...
            if ( problemList != null )
                LOGGER.log( Level.INFO,  "successfully loaded all Problems" );
        }
        catch( Throwable exc ) {
