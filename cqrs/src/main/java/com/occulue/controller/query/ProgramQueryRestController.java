...
public class ProgramQueryRestController extends BaseSpringRestController {
    ...
    public Program load( @RequestParam(required=true) UUID programId ) {
    	...
    	try {  
    		entity = ProgramBusinessDelegate.getProgramInstance().getProgram( new ProgramFetchOneSummary( programId ) );
        }
        catch( Throwable exc ) {
    ...
    public List<Program> loadAll() {                
    	...
    	try {
            ...
            if ( programList != null )
                LOGGER.log( Level.INFO,  "successfully loaded all Programs" );
        }
        catch( Throwable exc ) {
