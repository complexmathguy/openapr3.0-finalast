...
public class ValuesMapCommandRestController extends BaseSpringRestController {
     ...
     */
	@PostMapping("/create")
    public CompletableFuture<UUID> create( @RequestBody(required=true) CreateValuesMapCommand command ) {
     ...
     */
	@PutMapping("/update")
    public CompletableFuture<Void> update( @RequestBody(required=true) UpdateValuesMapCommand command ) {
		...
		try {                        	        
	    	...
	    	LOGGER.log( Level.WARNING, "ValuesMapController:update() - successfully update ValuesMap - " + exc.getMessage());        	
	    }		
    ...
    public CompletableFuture<Void> delete( @RequestParam(required=true) UUID valuesMapId  ) {
    	...
    	try {
    		...
    		LOGGER.log( Level.WARNING, "Successfully deleted ValuesMap with key " + command.getValuesMapId() );
        }
        catch( Throwable exc ) {
...
//************************************************************************
    ...
    private static final Logger LOGGER = Logger.getLogger(ValuesMapCommandRestController.class.getName());
