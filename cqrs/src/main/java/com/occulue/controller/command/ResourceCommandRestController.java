...
public class ResourceCommandRestController extends BaseSpringRestController {
     ...
     */
	@PostMapping("/create")
    public CompletableFuture<UUID> create( @RequestBody(required=true) CreateResourceCommand command ) {
     ...
     */
	@PutMapping("/update")
    public CompletableFuture<Void> update( @RequestBody(required=true) UpdateResourceCommand command ) {
		...
		try {                        	        
	    	...
	    	LOGGER.log( Level.WARNING, "ResourceController:update() - successfully update Resource - " + exc.getMessage());        	
	    }		
    ...
    public CompletableFuture<Void> delete( @RequestParam(required=true) UUID resourceId  ) {
    	...
    	try {
    		...
    		LOGGER.log( Level.WARNING, "Successfully deleted Resource with key " + command.getResourceId() );
        }
        catch( Throwable exc ) {
	...
	public void addToAttributes( @RequestBody(required=true) AssignAttributesToResourceCommand command ) {
		try {
	...
	public void removeFromAttributes( 	@RequestBody(required=true) RemoveAttributesFromResourceCommand command )
	{		
		try {
	...
	public void removeFromTargets( 	@RequestBody(required=true) RemoveTargetsFromResourceCommand command )
	{		
		try {
...
//************************************************************************
    ...
    private static final Logger LOGGER = Logger.getLogger(ResourceCommandRestController.class.getName());
