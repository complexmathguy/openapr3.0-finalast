...
public class NotifierCommandRestController extends BaseSpringRestController {
     ...
     */
	@PostMapping("/create")
    public CompletableFuture<UUID> create( @RequestBody(required=true) CreateNotifierCommand command ) {
     ...
     */
	@PutMapping("/update")
    public CompletableFuture<Void> update( @RequestBody(required=true) UpdateNotifierCommand command ) {
		...
		try {                        	        
	    	...
	    	LOGGER.log( Level.WARNING, "NotifierController:update() - successfully update Notifier - " + exc.getMessage());        	
	    }		
    ...
    public CompletableFuture<Void> delete( @RequestParam(required=true) UUID notifierId  ) {
    	...
    	try {
    		...
    		LOGGER.log( Level.WARNING, "Successfully deleted Notifier with key " + command.getNotifierId() );
        }
        catch( Throwable exc ) {
...
//************************************************************************
    ...
    private static final Logger LOGGER = Logger.getLogger(NotifierCommandRestController.class.getName());
