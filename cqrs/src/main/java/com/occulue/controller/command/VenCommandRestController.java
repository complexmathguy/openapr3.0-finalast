...
public class VenCommandRestController extends BaseSpringRestController {
    ...
    public CompletableFuture<Void> update( @RequestBody(required=true) UpdateVenCommand command ) {
		...
		try {                        	        
	    	...
	    	LOGGER.log( Level.WARNING, "VenController:update() - successfully update Ven - " + exc.getMessage());        	
	    }		
	...
	public void removeFromAttributes( 	@RequestBody(required=true) RemoveAttributesFromVenCommand command )
	{		
		try {
	...
	public void removeFromResources( 	@RequestBody(required=true) RemoveResourcesFromVenCommand command )
	{		
		try {
