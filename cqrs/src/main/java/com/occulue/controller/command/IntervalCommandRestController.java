...
public class IntervalCommandRestController extends BaseSpringRestController {
     ...
     */
	@PostMapping("/create")
    public CompletableFuture<UUID> create( @RequestBody(required=true) CreateIntervalCommand command ) {
     ...
     */
	@PutMapping("/update")
    public CompletableFuture<Void> update( @RequestBody(required=true) UpdateIntervalCommand command ) {
		...
		try {                        	        
	    	...
	    	LOGGER.log( Level.WARNING, "IntervalController:update() - successfully update Interval - " + exc.getMessage());        	
	    }		
    ...
    public CompletableFuture<Void> delete( @RequestParam(required=true) UUID intervalId  ) {
    	...
    	try {
    		...
    		LOGGER.log( Level.WARNING, "Successfully deleted Interval with key " + command.getIntervalId() );
        }
        catch( Throwable exc ) {
	...
	public void unAssignIntervalPeriod( @RequestBody(required=true)  UnAssignIntervalPeriodFromIntervalCommand command ) {
		try {
	...
	public void removeFromPayloads( 	@RequestBody(required=true) RemovePayloadsFromIntervalCommand command )
	{		
		try {
...
//************************************************************************
    ...
    private static final Logger LOGGER = Logger.getLogger(IntervalCommandRestController.class.getName());
