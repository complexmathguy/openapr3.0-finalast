...
public class ProblemCommandRestController extends BaseSpringRestController {
     ...
     */
	@PostMapping("/create")
    public CompletableFuture<UUID> create( @RequestBody(required=true) CreateProblemCommand command ) {
     ...
     */
	@PutMapping("/update")
    public CompletableFuture<Void> update( @RequestBody(required=true) UpdateProblemCommand command ) {
		...
		try {                        	        
	    	...
	    	LOGGER.log( Level.WARNING, "ProblemController:update() - successfully update Problem - " + exc.getMessage());        	
	    }		
...
//************************************************************************
    ...
    private static final Logger LOGGER = Logger.getLogger(ProblemCommandRestController.class.getName());
